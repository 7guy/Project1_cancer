...
def predict_scenario(current_data, changes, cancer_type):
    """
    app.py에서 'changes' 딕셔너리를 직접 받으므로 구조 변경
    """
    # 1. 기존 데이터 복사 (원본 데이터 오염 방지)
    scenario_data = current_data.copy()
    
    # 2. 변경 사항이 있을 경우 한꺼번에 업데이트
    if changes and isinstance(changes, dict):
        # * [수정] GPT가 준 값을 덮어쓰기
        scenario_data.update(changes)
        
    # 3. 변경된 데이터로 '기존 예측 함수' 재사용
    # * 여기서 에러가 난다면 predict_cancer_risk 내부의 데이터 변환 로직을 확인해야 함
